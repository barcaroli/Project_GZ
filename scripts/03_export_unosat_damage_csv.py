#!/usr/bin/env python3
"""
Name: 03_export_unosat_damage_csv.py
Function: Export UNOSAT damage point features from FileGDB to a normalized CSV.
Input datasets:
- data/UNOSAT_GazaStrip_CDA_11October2025.gdb (or another UNOSAT FileGDB)
Processing Logic:
- Open the requested FileGDB layer via Fiona.
- Detect damage class field (or use user override).
- Read point geometries and map raw damage codes to textual classes.
- Write a flat CSV suitable for downstream joins/spatial aggregation.
Output datasets:
- out/unosat_damage.csv (default), with columns:
  osm_id, lon, lat, damage_level, damage_code
"""
...
# Edit these defaults to your local paths.
DEFAULT_GDB_PATH = r"D:\\Google Drive\\Societa geografica\\Palestina\\Workflow2\\data\\UNOSAT_GazaStrip_CDA_11October2025.gdb"
DEFAULT_OUTPUT_CSV = r"D:\\Google Drive\\Societa geografica\\Palestina\\Workflow2\\out\\unosat_damage.csv"
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        help=f"Path to .gdb folder (default: {DEFAULT_GDB_PATH})",
    )
    p.add_argument(
        "output_csv",
        nargs="?",
